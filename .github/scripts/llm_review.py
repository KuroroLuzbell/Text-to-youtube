...
def has_blocking_issues(review_text: str, fail_severity: str) -> bool:
    ...
    for severity_str in found_severities:
        ...
        if found_level and found_level >= fail_level:
            print(
                f"Hallazgo de bloqueo detectado: Se encontró un problema de gravedad '{severity_str}', que cumple o supera el umbral de fallo '{fail_severity}'."
            )
            return True
...
def analyze_code_with_llm(files: List[str], provider: str, api_key: str) -> List[str]:
    ...
    if provider.lower() != "gemini":
        print(
            f"Error: Este script actualmente solo es compatible con el proveedor 'gemini'. Se encontró '{provider}'."
        )
        sys.exit(1)
...
def main():
    parser = argparse.ArgumentParser(
        description="Analiza cambios en el código con un LLM y opcionalmente falla basado en la severidad."
    )
    parser.add_argument(
        "--provider",
        type=str,
        default=os.getenv("LLM_PROVIDER", "gemini"),
        help="Proveedor de LLM (gemini, openai, etc)",
    )
    parser.add_argument(
        "--api-key",
        type=str,
        default=os.getenv("LLM_API_KEY"),
        help="Clave de API para el proveedor de LLM",
    )
